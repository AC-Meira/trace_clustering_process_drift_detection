...
import hdbscan
from source import clustering_utils as c_utils
...
def compare_clusterings(interation_1, interation_2):
    ...
    # Run Monic
    # print("Comparing ", resp_1['i'], "with ", resp_2['i'])
    # print("i clusters:", len(np.unique(resp_1['y_pred'])) )
    # print("j clusters:", len(np.unique(resp_2['y_pred'])) )
    # monic_object = c_utils.Monic(resp_1['X'], resp_1['y_pred'], resp_2['X'], resp_2['y_pred']
    #                               , t=1, age=None, threshold_match=0.3, threshold_split=0.25)
    # print('matches:', monic_object.matches)
    # print('survivors:',monic_object.survivors)
    # print('splits:',monic_object.splits)
    # print('absorptions:',monic_object.absorptions)
    # print('max_overlaps:',monic_object.max_overlaps)
    # print('overlap_matrix:',monic_object.overlap_matrix)
...
def run_offline_clustering_window(
    ...
    X_without_outliers = pd.DataFrame()
    ...
    for i in loop:
        ...
        if dim_reduction_function is not None:
            if i == 0:
                drf = dim_reduction_function
                X = pd.DataFrame(drf.fit_transform(X), columns=["X"+str(n+1) for n in range(0,drf.n_components)]) 
            else:
                X = pd.DataFrame(drf.transform(X), columns=["X"+str(n+1) for n in range(0,drf.n_components)])
        
            # Data Scaler
            X_columns = X.columns
            if i == 0:
                scaler = MinMaxScaler() # PowerTransformer, MinMaxScaler
        ...
        #     , min_grad_norm=1e-07
        #     , metric=distance_metric[0]
        #     , init='random'
        #     , verbose=0
        #     , method='barnes_hut'
        #     , angle=0.5
        #     , random_state=42)
        # X = pd.DataFrame(tsne.fit_transform(X), columns=['X','Y'])#,'Z'])
        ...
        #           , random_state=None)
        # X = pd.DataFrame(mds.fit_transform(X), columns=['X','Y'])#,'Z'])
        ...
        #             , iterated_power='auto'
        #             , random_state=42)
        #     X = pd.DataFrame(pca.fit_transform(X), columns=['X1','X2'])#,'X3','X4','X5'])
        # else:
        #     X = pd.DataFrame(pca.transform(X), columns=['X1','X2'])
        ...
        #             , tol=0.0
        #             , random_state=42)
        #     X = pd.DataFrame(svd.fit_transform(X), columns=['X1','X2'])
        # else:
        #     X = pd.DataFrame(svd.transform(X), columns=['X1','X2'])
            
        # LocallyLinearEmbedding transform
        # if i == 0:
        #     lle = LocallyLinearEmbedding(
        #             n_neighbors= (window-1) 
        #             , n_components=2
        #             , reg=0.001
        ...
        #             , random_state=42)
        #     X = pd.DataFrame(lle.fit_transform(X), columns=['X1','X2']) #,'X3'
        # else:
        #     X = pd.DataFrame(lle.transform(X), columns=['X1','X2']) #,'X3'
        ...
        # Add X in df to be returned
        X['iteration'] = i
        X.reset_index(inplace=True)
        X.set_index(['iteration', 'index'], inplace=True)
        X_without_outliers = X_without_outliers.append(X)
        ...
        r["n_representation_distinct"] = len(np.unique(np.round(X,2), axis=0))
    ...
    # Remove X from df for space
    run_df.drop('X', axis=1, inplace=True)
    ...
    return all_metrics, X_without_outliers